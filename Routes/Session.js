...
var duration = 3600000;     // One hour in milliseconds
var cookieName = 'ZuumAuth'; // Cookie key for authentication tokens

const CRYPTO_BYTES = 16;
...
//|duration|
// (though the router, below, will check anyway to prevent hacking), and
// will not be
// shown by the browser to the user, again to prevent hacking.
exports.makeSession = function makeSession(user, res) {
   var authToken = crypto.randomBytes(CRYPTO_BYTES).toString('hex');
...
// Export a router that will find any Session associated with |req|,
//based on cookies,
// delete the Session if it has timed out, or attach the Session to |req|
//if it's current
// If |req| has an attached Session after this process, then down-chain
//routes will
